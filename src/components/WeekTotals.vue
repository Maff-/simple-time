        ...
        <div class="d-flex flex-wrap gap-3">
            ...
            <div v-if="!disableSubmissions" class="d-flex align-items-center gap-1">
                <span v-for="(details, day) in submitDayStatus"
                      ...
                      :class="{'bg-gray text-dark': details.status === 'submitted' || (details.inFuture && !details.loggedHours), 'bg-danger text-white': (details.status === 'no_registrations' && !details.inFuture), 'bg-success text-white': details.status === 'yet_to_turn_in' && details.loggedHours >= details.hours, 'bg-warning text-white': details.status === 'yet_to_turn_in' && details.loggedHours < details.hours}"
